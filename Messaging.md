...
Optional sub-types are also serialized as a TLV fields.
Absence of the field is denoted by having a zero length.
...
`field_type` values should start at `0` and increase by `1` for every new `Optional` field.
...
The `type` field enables supporting future siblings of `oracle_announcements`.
At this moment only a single variant is defined.
